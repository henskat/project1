//writes nav bar
document.write(
  `<span class = "title"><a href = "index.html">Pacifica</a></span>`
);
document.write(`<div id="menu-button">`);
document.write(`<svg viewBox="0 0 100 80" width="30" height="30">`);
document.write(`<rect fill="white" width="100" height="15"></rect>`);
document.write(`<rect fill="white" y="30" width="100" height="15"></rect>`);
document.write(`<rect fill="white" y="60" width="100" height="15"></rect>`);
document.write(`</svg>`);
document.write(`</div>`);
document.write(`<nav id = "navigation">`);
document.write(`<ul class = "categories">`);
document.write(`<a href = "index.html"><li class = "category">Home</li></a>`);
document.write(
  `<a href = "about-us.html"> <li class = "category">About Us</li></a>`
);
document.write(`<li class =  "category">`);
document.write(`<a href = "#"> Services </a>`);
document.write(`<ul class = "dropdown">`);
document.write(`<a href = "services-facials.html"><li>Facials</li></a>`);
document.write(`<a href = "services-massages.html"><li>Massages</li></a>`);
document.write(`<a href = "services-beauty.html"><li>Beauty</li></a>`);
document.write(`</ul>`);
document.write(`</li>`);
document.write(`<li class = "category"><a href = "#">Specials</a>`);
document.write(` <ul class = "dropdown">`);
document.write(`<a href = "specials-rewards.html"><li>Rewards</li></a>`);
document.write(`<a href = "specials-gift-cards.html"><li>Gift Cards</li></a>`);
document.write(`</ul>`);
document.write(`</li>`);
document.write(
  ` <a href = "contact-us.html"><li class = "category">Contact Us</li></a>`
);
document.write(`</ul>`);
document.write(`</nav>`);
